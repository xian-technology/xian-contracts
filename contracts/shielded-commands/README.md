...
- Commands can authorize a proof-bound `public_amount` budget. Allowlisted
  target adapters consume that budget through `adapter_spend_public(...)`
  during the active execution window.
- Hidden change payloads are proof-bound by per-output payload hashes, so
...
- The privacy boundary is sender privacy and relayed execution, not invisible
  ...
  observable on-chain.
- In this package model, `shielded-note-token` is the private value pool,
  `shielded-commands` is the anonymous execution coordinator, and
