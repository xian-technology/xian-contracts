...
- It passes the package XSC001 interface checker and exposes
...
- DEX integration requires setting the relevant pair-facing address or addresses
  as fee targets before initial liquidity deposits.
...
- The package tests use `con_pairs` as the fee target and
...
- The DEX router returns actual credited and received amounts for these
...
- Swaps should use the DEX fee-on-transfer path,
  ...
  sells.
- The helper contract path expects an explicit absolute deadline and uses
  wider slippage than a normal token because
